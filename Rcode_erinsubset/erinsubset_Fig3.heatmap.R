## PAM-clustering of samples and making a heatmap
# 1.7.16
# Anna M. Seekatz
...
# files used:
	...
	# erinfmt_genbar.txt
	# erinfmt_genfrac2p_ordered.txt
	# ERIN.UMFMT_metadata.txt
...
bar_ordered<- bar[order(bar$total_sample_n, bar$group, bar$patientID, bar$sampleID, bar$sample_n),]
#write.table(bar_ordered, 'erinfmt_genfrac.all_w.meta.txt',quote=FALSE,sep="\t", col.names=NA)
...
# files used:
	...
	# erinfmt_genfrac.all_w.meta.txt
...
names.subset<-read.table(file="Mothurfiles_erinsubset/erinsubset.0.03.shared", header=TRUE, row.names=2)
names<-as.character(rownames(names.subset))		# these are all of the subset samples
phylos<-read.table(file="erinfmt_genfrac.all_w.meta.txt", header=TRUE)
...
#for phylos:
postscript('ALLphylos.sub.erin.phylo_pamtest.ps',width=8,height=8,horizontal=TRUE)
for (nc in 2:10) {
plot(pam(js, k=nc))
}
dev.off()
...
#write.table(suberin.summary2, file="suberin_summary.phylotyped.txt", sep="\t", quote=FALSE, col.names=NA)


##-----------------------------------------
#### Part 3: Heatmap of data
...
# files used:
	# suberin_summary.phylotyped.txt
	# erinsubset.0.03.shared
...
shared<-read.table(file="Mothurfiles_erinsubset/erinsubset.0.03.shared", header=TRUE, row.names=2)
...
#sum2$patient_status<-paste(sum2$group2, sum2$index, sep="_")
sum2$color.clinical<-mapvalues(sum2$clinical_result, from = c("negative", "positive"), to = c("grey47","magenta"))

# add column metadata (taxonomy info):
taxonomy_file<-read.table(file="Mothurfiles_erinsubset/erinfmt.trim.contigs.good.unique.good.filter.unique.precluster.pick.pick.pick.an.unique_list.0.03.cons.taxonomy", header=TRUE)
...
heatmap.2(top40,
                    ...
                    labRow="",
					RowSideColors = as.character(sum2$color),
					ColSideColors = as.character(phyla.col)
					)
